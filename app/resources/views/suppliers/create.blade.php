...
<x-app-layout>
    <x-slot name="header">
        <x-page-header :title="'Add supplier'" :breadcrumbs="[
    ...
    <form method="POST" action="{{ route('suppliers.store') }}"
        ...
        <div>
            ...
            <input type="text" id="gstin" name="gstin" value="{{ old('gstin') }}" maxlength="15"
        ...
        <div>
            ...
            <select id="state_code" name="state_code"
                ...
                @foreach ($stateCodes as $code => $label)
                    <option value="{{ $code }}" @selected(old('state_code') == $code)>{{ $code }} — {{ $label }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <textarea id="address" name="address" rows="3"
                      class="mt-1 block w-full rounded-md border-slate-300 focus:border-brand-500 focus:ring-brand-500 @error('address') border-red-400 ring-1 ring-red-400 @enderror">{{ old('address') }}</textarea>
