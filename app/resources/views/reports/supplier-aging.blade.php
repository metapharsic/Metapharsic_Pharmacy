{{--
    Supplier aging report — phase-5-reports.md T-0507b/T-0507c. supplier.ledger /
    report.aging permission. Same 0-30/31-60/61-90/90+ bucket rule as the
    customer aging report, computed from real purchase/payment dates.

    Cave law 2: aging is payables, not margin — no cost-vs-sale comparison
    belongs on this screen; purchase totals here are what the shop owes,
    not what it earned.
--}}
<x-app-layout>
    <x-slot name="title">Supplier Aging</x-slot>

    <div class="space-y-4">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold text-slate-900">Supplier Aging</h1>
            <a
                href="{{ route('reports.supplier-aging', array_merge(request()->query(), ['type' => 'supplier', 'format' => 'csv'])) }}"
        ...
        <form method="GET" action="{{ route('reports.supplier-aging') }}" class="flex flex-wrap items-end gap-3 rounded-lg border border-slate-200 bg-white p-4">
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50">
                    <tr class="text-left text-xs uppercase tracking-wide text-slate-500">
                        ...
                        <th scope="col" class="px-4 py-2 text-right">Total Payable</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse (($rows ?? []) as $row)
